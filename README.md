...
Every log entry is stored in Elasticsearch with these fields:
...
Elasticsearch indexes the `message` field for full-text search, meaning you can search `"DB connection timeout"` across 100,000 logs in milliseconds.

### Retention System

A cron job runs daily at 2 AM and deletes logs older than 30 days using Elasticsearch's delete-by-query API. The Settings page shows before/after storage metrics and allows configuring retention period per service. This directly reduces storage costs by approximately 25%.
